...
WRAP_DOC_STRINGS = True  # 2007 May 25
DOUBLE_QUOTED_STRINGS = False  # 2006 Dec 05
SINGLE_QUOTED_STRINGS = True  # 2007 May 01
            ...
            *tokenize* module does not.  Thus, although the parts are
            ...
            try:
                while True:
                    ...
                    (prev_token_type, prev_token_string, prev_start,
                     prev_end, prev_line) = prev_item
                    if prev_token_type in [tokenize.STRING]:
                        ...
                        while True:
                            ...
                            (next_token_type, next_token_string,
                             next_start, next_end, next_line) = \
                                next_item
                            if next_token_type in [tokenize.STRING]:
                                if prev_token_string[-1] \
                                    == next_token_string[ZERO]:
                                    prev_token_string = \
    prev_token_string[:-1] + next_token_string[1:]
                                    on1 = False
                            else:
                                if on1:
                                    pass
        ...
        prev_token_string, prev_start, prev_end, prev_line)
                                    yield prev_item
                                    break
        ...
        for (token_type, token_string, start, end, line) in lines:
            ...
            if token_type in [tokenize.COMMENT, tokenize.NL]:
                ...
                try:
                    ...
                    if encoded == original or encoded \
...
class NodeStr(Node):
    ...
    def get_as_repr(self):  # 2007 May 01
        original_values = \
            COMMENTS.literal_pool.get(repr(self.get_as_str()), [])  # 2010 Mar 10
...
class NodeDiscard(Node):
    ...
    def put(self, can_split=False):
        if isinstance(self.expr, NodeConst) \
            and not KEEP_UNASSIGNED_CONSTANTS:  # 2010 Mar 10
            pass
        else:
            self.line_init()
            self.expr.put(can_split=can_split)
...
class NodeIf(Node):
    ...
    def __init__(
        ...
        Node.__init__(self, indent, lineno)
        self.tests = [(transform(indent, lineno, expr),
                      transform(indent + 1, lineno, stmt)) for (expr,
                      stmt) in tests]
        self.else_ = transform(indent + 1, lineno, else_)
        return
